<?php
	...
	if ( isset($_POST['btn-signup']) ) {
		...
		$salesemail = strip_tags($salesemail);
		...
		*/
		...
		if ( !filter_var($salesemail,FILTER_VALIDATE_EMAIL) ) {
			$error = true;
			$salesemailError = "Please enter valid sales email address.";
		} else {
			// check email exist or not
			$query = "SELECT email FROM register WHERE email='$salesemail' and register_type='sales'";
			$result = mysql_query($query);
			$count = mysql_num_rows($result);
			if($count==0){
				$error = true;
				$salesemailError = "Provided sales Email.";
			}
		}
		...
		if( !$error ) {
			...
			$query = "INSERT INTO register(date_created,name,email,password,cpassword,gst,pan,tel,address,register_type,salesemail) VALUES(NOW(),'$name','$email','$password','$cpass','$gst','$tel','$pan','$address','distributor','$salesemail')";
			...
			<form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" autocomplete="off">
				...
				<div class="col-md-12">
					...
					<div class="form-group">
						<div class="input-group">
						<span class="input-group-addon"><span class="glyphicon glyphicon-envelope"></span></span>
						<input type="email" name="salesemail" class="form-control" placeholder="Enter Refered sales user email" maxlength="40" value="<?php echo $salesemail ?>" />
						</div>
						<span class="text-danger"><?php echo $salesemailError; ?></span>
					</div>
					...
					<div class="form-group">
						...
						<input type="text" name="gst" class="form-control" placeholder="Enter gst number" maxlength="50" value="<?php echo $gst ?>" />
						</div>
					...
					<div class="form-group">
						<div class="input-group">
						<span class="input-group-addon"><span class="glyphicon glyphicon-globe"></span></span>
						<input type="text" name="pan" class="form-control" placeholder="Enter pan number" maxlength="50" value="<?php echo $pan ?>" />
						</div>
					...
					<div class="form-group">
						...
						<textarea name="address" class="form-control" placeholder="Enter Address" maxlength="50"/></textarea>
						</div>
